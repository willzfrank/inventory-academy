...
import OnboardingStatusBadge from "@/components/onboarding-management/OnboardingStatusBadge";
import type { OnboardingManagementListRow, OnboardingStatus } from "@/data/onboardingManagementList";
...
export default function OnboardingManagementPanel({
  ...
  rows: OnboardingManagementListRow[];
}) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      if (status !== "all" && r.onboardingStatus !== status) return false;
      if (!q) return true;
      return (
    ...
    <div className="rounded-lg border border-[#EAECF0] bg-white p-6 shadow-sm">
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0 flex-1">
          <Input
            id="onboarding-management-search"
            type="search"
        ...
        <div className="flex w-full shrink-0 items-center gap-2 lg:w-auto">
          <Select
            id="onboarding-management-status"
            label="Status"
            inlineLabel
            value={status}
            onChange={(e) => {
              setStatus(e.target.value as "all" | OnboardingStatus);
              setPage(1);
            }}
          ...
          <tbody>
            {pageRows.map((row, i) => (
              ...
              >
                ...
                <td className={bodyCellClass}>{row.approvalDate}</td>
                <td className={bodyCellClass}>
                  <OnboardingStatusBadge status={row.onboardingStatus} />
                </td>
                <td className={bodyCellClass}>
                  <Link
                    to={`/onboarding-mgmt/${encodeURIComponent(row.vendorId)}`}
